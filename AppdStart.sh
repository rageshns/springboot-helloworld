...
main() {
  ...
  then
    ...
    echo "must include a AppdConfig.env file"
    ...
    export $(cat AppdConfig.env | xargs)
    ...
    if [[ "x$CONTROLLER_HOST" == "x" || "x$CONTROLLER_PORT" == "x" || "x$CONTROLLER_SSL_ENABLED" == "x" || "x$APPLICATION_NAME" == "x" || "x$ACCOUNT_ACCESS_KEY" == "x" ]]; then
      ...
      docker run -d --env-file=AppdConfig.env $VOLUME_MOUNT --expose "9090" --dns="8.8.8.8" --volume=/:/hostroot:ro --volume "/var/run/docker.sock:/var/run/docker.sock" \
